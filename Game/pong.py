...
class Pong(object):
    ...
    def left_paddle(self, ai=1):  # set ai = None, if there are two human users
        ...
        # limit the possible y positions so that paddle doesn't go off window
    ...
    def update_puck(self):
        ...
        if (self.ball_rect.right >= self.display_width - self.paddle_width) and (
            ...
            if (self.ball_rect.top - 10 >= self.right_paddle_mv) and (
                    self.ball_rect.bottom - 10 <= self.right_paddle_mv + 75):
                self.direction[0] = -1
                self.play_sound()
        ...
        if (self.ball_rect.left <= self.paddle_width) and (self.ball_rect.left >= 0):
            if (self.ball_rect.top - 10 >= self.left_paddle_mv) and (
                    self.ball_rect.bottom - 10 <= self.left_paddle_mv + 75):
                self.direction[0] = 1
                self.play_sound()
